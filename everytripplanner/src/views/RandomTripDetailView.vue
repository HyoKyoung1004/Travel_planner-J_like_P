  ...
  <div>
    ...
    <div style="display: flex; justify-content: center" v-if="changeObject">
      ...
      <b-nav class="child">
        ...
        <b-button variant="success" active @click="viewAttractionList(15)"
          >축제공연행사 위주</b-button
        >
...
export default {
  ...
  methods: {
    viewAttractionList(idx) {
      axios
        .get(`http://localhost:9999/trip/attract/random/${this.sido}/${this.gugun}/${idx}`)
        .then((response) => {
    ...
    shoot(randomplan) {
      ...
      axios.post("http://localhost:9999/trip/plan/realPlanAdd", shootPlan).then((resp) => {
        console.log(resp);
